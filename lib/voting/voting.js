(function() {
    ...
    define(deps, function(
       ...
       */
      Voting.prototype.eventsPullRequestOpened = (self) => function eventsPullRequestOpened(data) {
       ...
       */
      Voting.prototype.eventsPullRequestClosed = (self) => function eventsPullRequestClosed(data) {
        // Don't process events for other repositories.
        /*if (data.repository.name !== this.repo.repo ||
            data.repository.owner.login !== this.repo.user) {
          return;
       ...
       */
      Voting.prototype.eventsCommentCreated = (self) => function eventsCommentCreated(data) {
        // Don't process events for other repositories.
        /*if (data.repository.name !== this.repo.repo ||
            data.repository.owner.login !== this.repo.user) {
          return;
      ...
      Voting.prototype.initializeEvents = function initializeEvents() {
        events.on('github.pull_request.opened', this.eventsPullRequestOpened(this));
        events.on('github.pull_request.closed', this.eventsPullRequestClosed(this));
        events.on('github.comment.created', this.eventsCommentCreated(this));
      };
      ...
      Voting.prototype.getVoteStartedComment  = function getVoteStartedComment(pr, cb) {
        for (var i = 0; i < pr.comments.length; i++) {
          var repo_user = 'minhoryang';  // TODO(minhoryang)
          var postedByMe = pr.comments[i].user.login === repo_user;
          /* FIXME
...
console.log(this.votingConfig.guaranteedResult);
